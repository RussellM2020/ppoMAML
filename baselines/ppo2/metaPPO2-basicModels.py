...
def setupInnerUpdates(metaCtr, taskCtr):

    with tf.name_scope(taskScope+"/Objective"):
        neglogpac = taskCtr.train_model.pd.neglogp(A)
        entropy = tf.reduce_mean(taskCtr.train_model.pd.entropy())

        vpred = taskCtr.train_model.vf
        vpredclipped = OLDVPRED + tf.clip_by_value(train_model.vf - OLDVPRED, - CLIPRANGE, CLIPRANGE)
        vf_losses1 = tf.square(vpred - R)
        vf_losses2 = tf.square(vpredclipped - R)
        vf_loss = .5 * tf.reduce_mean(tf.maximum(vf_losses1, vf_losses2))
...
def learn(*, policy, env, nsteps, total_timesteps, ent_coef, lr,
    ...
    def setupCompGraph():
        ...
        metaController = make_model("Meta")
